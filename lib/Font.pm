...
require Exporter;
our @ISA = qw(Exporter);
our @EXPORT = qw(load_font textWidth textHeight);
...
sub new
{
	...
	my $font = {};
	my $default = {"font-szie"=>12,"font-family"=>"arial","fill"=>"black","font-weight"=>"normal","font-style"=>"normal"};
...
sub textWidth
{
	...
	$num = 0 if (not exists $init->{$ffam}->{$num});
	my $info = new PostScript::FontMetrics ($init->{$ffam}->{$num});	
